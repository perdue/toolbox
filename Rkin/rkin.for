C	PROGRAM RKIN
C angle in degrees, energy in MeV, inreac is a char array, inex is excitation energy of the final state
	subroutine rkin(finang,finen,inreac,finex,fQ,fen3,flabang3,fcmang3,fen4,
     +               flabang4,fratiop)
C
...
C
	...
	CHARACTER*25 inreac
	REAL*8 ALLDAT1(266,13)
	REAL*8 ALLDAT2(266,13)
	REAL*8 ALLDAT3(266,13)
C
	COMMON AM1, AM2, AM3, AM40, AM1SQ, AM2SQ, AM3SQ, E1, EIN,
	1 P1, ELAB, P1CHK, P1CM, GAMMA, BETA, D, TCM
	COMMON /SEAN/ALLDAT1, ALLDAT2, ALLDAT3

C	WRITE(*,*) 'INPUTS: ',finang, ' ',finen, ' ', inreac, ' ' ,finex
C	STOP
C
...
10	CONTINUE
	NANG=1
C	IF(NANG.GT.MAXANG) NANG = MAXANG
C	WRITE(LOUT,1014)NANG
C	READ(LIN,*)(ANGSTO(I),I=1,NANG)
C	ENDIF
C	WRITE(LOUT,1016)
C Number of energies
C	READ(LIN,*) NE
	   NE=1
C	WRITE(LOUT,1018)
C	READ(LIN,*) (T1(I),I=1,NE)
	   T1(1)=finen
C	WRITE(LOUT,1020)
...
C	WRITE(LOUT,1026)
	EX(1,1)=finex
C	READ(LIN,*) (EX(I,J),J=1,NEX(I))
40	CONTINUE
C	WRITE(LOUT,1028)
C  END DATA READ-IN
C
C	OPEN(UNIT=FILE,TYPE='UNKNOWN',NAME='rkin.out')
C
...
C
	...
	DO 180 K=1,NEX(J)
C	WRITE(FILE,1030) REACTN(J)
C	WRITE(FILE,1032) (L,L=1,4)
	Q = Q0 - EX(J,K)
C  Pass variables to output variables	
	fQ=Q
C	WRITE(FILE,1034) (AM(J,L),L=1,4),EX(J,K),T1(I),Q,TCM
C	WRITE(FILE,1036)
	DO 180 L=1,NANG
	ANGLAB = finang
...
C  Pass variables to output variables	
	...
	flabang4=ANGL4P
	fen4=T4P
C	WRITE(*,*) "IN RKIN.FOR: ",ANGLAB,ANG3P,T3P,ANGL4P,T4P,RATIOP
C	WRITE(FILE,1038) ANGLAB,ANG3P,T3P,ANGL4P,T4P,RATIOP
	GO TO 180
151	CONTINUE
C	WRITE(FILE,1040)
	GO TO 200
152	CONTINUE
C	WRITE(FILE,1042)ANGLAB
180	CONTINUE
200	CONTINUE
C	STOP
	RETURN 
...
1010	FORMAT( ' Type 0(Zero) to input initial, final, and increment',
	1 ' angles OR',/,' type in the number of angles to be typed in ',
	2 '(200 max.): ',$)
1012	FORMAT(' Type in initial angle, final angle, angle increment: ',$)
1014	FORMAT(' Type in the ',I3, ' angles: ',$)
1016	FORMAT(' Type in number of energies (integer) : ',$)
1018	FORMAT(' Type in energies : ',$)
...
1026	FORMAT(' Type in excitation energies of residual nucleus : ',$)
...
C	WRITE(*,*) 'END SRKIN:',EX,ANGLAB,ANG3P,T3P,SHIFTP, ANGL4P,
C      1    T4P,RATIOP,QP,ANG3M,T3M, ANGL4M, T4M,SHIFTM,RATIOM,QM
	RETURN
	END
...
C
C	WRITE(LOUT,5)
C5   	FORMAT (' Input reaction in form -  27Al(p,p)27Al  ',
C	1 /,' (Residual nucleus may be omitted.) ',$)
C10	  READ(LIN,15) RCTIN
C15   	FORMAT (A25)
C	REACTN=RCTIN
C
...
C READ MASS83A.DAT
C
C      OPEN (UNIT=9,FILE='/home/capture/stave/Duke/rkin/MASS83A.DAT'
C     1  ,STATUS='OLD')

C      do 10 k=1,A
C          READ (9,*) AA,((DATA(I,J),J=1,3),I=1,13)
C   10 continue
...
C READ MASS83A.DAT
C
      OPEN (UNIT=9,FILE='MASS83A.DAT'
     1  ,STATUS='OLD')

      do 10 k=1,UPLIM
          READ (9,*) AA,((DATA(I,J),J=1,3),I=1,13)
	  do 2 i=1,13
             ALLDAT1(k,i)=DATA(I,1)
             ALLDAT2(k,i)=DATA(I,2)
             ALLDAT3(k,i)=DATA(I,3)
 2	     CONTINUE
   10 continue
